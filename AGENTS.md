# Working in rust-fs-erofs (agent guide)

Pure-Rust EROFS driver exposing a C ABI, validated against erofs-utils — `mkfs.erofs`, `fsck.erofs` strict and `dump.erofs`. This file is the fast path
...
chore testscripts   # the shell guards
chore build
chore lint          # fmt, the agent-core check, clippy
chore staticlib     # what the app links
```

CI runs `test` and `validate-mkfs-bin`, aggregated by `ci-ok`.

## The oracle is erofs-utils

`mkfs.erofs` builds the images, `fsck.erofs` (strict) and `dump.erofs` grade
them. `validate-mkfs-bin` is the job that matters: an image this crate accepts
must be one `fsck.erofs` accepts, and an image `mkfs.erofs` produced must be one
this crate reads identically.

Two things about the local build, because they cost time otherwise: erofs-utils
